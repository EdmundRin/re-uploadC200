...
def insertValues(cursor, tableName, valueString):
    ...
    """
    # TODO
    inserQuery = "INSERT INTO {} VALUES ({})".format(tableName, valueString)
...
if __name__ == "__main__":
    ...
    query = queryStatement("Owner.Name, Animal.Name", "Owner, Animal", "Animal.Type == 'Cat' AND Animal.ID == Owner.AnimalID") # TODO: Fill this in
    # query = queryStatement("SELECT Owner.Name, Animal.Name FROM Owner, Animal WHERE Animal.Type = 'Cat' AND Animal.ID = Owner.AnimalID")
    ...
    query = queryStatement("Owner.Name, Animal.Name", "Owner, Animal", "Animal.Age <= 3 AND Animal.ID == Owner.AnimalID") # TODO: Fill this in
    # query = queryStatement("SELECT Owner.Name, Animal.Name FROM Owner, Animal WHERE Animal.Age <= 3 AND Animal.ID = Owner.AnimalID")
    for x in theCursor.execute(query):
        print(x)
